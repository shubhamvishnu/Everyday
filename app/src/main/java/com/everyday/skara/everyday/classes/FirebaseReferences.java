package com.everyday.skara.everyday.classes;

public class FirebaseReferences {
    public final static String FIREBASE_USER_DETAILS = "user_details/";
    public final static String FIREBASE_BOARDS = "boards/";
    public final static String FIREBASE_BOARDS_INFO = "boards_info/";
    public final static String FIREBASE_OTHER_BOARDS_INFO = "other_boards_info/";
    public final static String FIREBASE_FEEDBACK = "feedback/";
    public final static String FIREBASE_PERSONAL_BOARD_PROD = "personal_boards/productivity_board/";
    public final static String FIREBASE_PERSONAL_BOARD_FINANCIAL = "personal_boards/financial_board/";

}